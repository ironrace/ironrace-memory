...
fn replay_over_synthetic_repo_emits_fact_at_commit_rows() {
    ...
    // 2 facts (function signature + public symbol `ten`) × 2 commits = 4 rows.
...
#[test]
fn replay_extracts_field_and_test_assertion_facts_at_t0() {
    let tmp = tempfile::tempdir().unwrap();
    let p = tmp.path();
    let g = |args: &[&str]| {
        let s = std::process::Command::new("git")
    ...
    std::fs::write(
        p.join("src/lib.rs"),
        b"pub struct Config { pub limit: usize }\npub fn limit() -> usize { 10 }\n#[test]\nfn limit_is_ten() { assert_eq!(limit(), 10); }\n",
    )
    .unwrap();
    std::fs::write(p.join("README.md"), b"Call `limit` from tests.\n").unwrap();
    g(&["add", "."]);
    g(&[
        "-c",
        "user.name=t",
    ...
    std::fs::write(
        p.join("src/lib.rs"),
        b"pub struct Config { pub limit: usize }\npub fn limit() -> usize { 11 }\n#[test]\nfn limit_is_ten() { assert_eq!(limit(), 11); }\n",
    )
    .unwrap();
    g(&["add", "."]);
    g(&[
    ...
    let rows = Replay::run(&cfg).unwrap();
    assert!(
        rows.iter().any(|r| r.fact_id.starts_with("Field::")),
        "expected at least one Field row, got {rows:?}"
    );
    assert!(
        rows.iter()
            .any(|r| r.fact_id.starts_with("TestAssertion::")),
        "expected at least one TestAssertion row, got {rows:?}"
    );
    assert!(
        rows.iter().any(|r| r.fact_id.starts_with("DocClaim::")),
